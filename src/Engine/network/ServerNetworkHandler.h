...
public:
	...
	bool isWhitelist;					// 32
	Whitelist *whitelist;				// 36
	char filler1[24];					// 40
	bool visible;						// 64
	BatchPacket *batchPacket;			// 68
	Player *player;						// 72
	std::string serverName;				// 76
	...
	ServerPlayer *_getServerPlayer(const RakNet::RakNetGUID &);
	...
	std::unique_ptr<ServerPlayer> createNewPlayer(const RakNet::RakNetGUID &, const LoginPacket *);
